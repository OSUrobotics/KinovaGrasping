...
def eval_policy(policy, env_name, seed, requested_shapes, eval_episodes=200, compare=True):
	num_success=[0,0]
	...
	if compare:
		...
		for i in range(40):
			...
			success=0
			#eval_env = gym.make(env_name)
			state, done = eval_env.reset(start_pos=[x,y],env_name="eval_env",shape_keys=requested_shapes), False
			cumulative_reward = 0
		...
		for i in range(eval_episodes):
			success=0
			#eval_env = gym.make(env_name)
			state, done = eval_env.reset(shape_keys=requested_shapes,env_name="eval_env"), False
			cumulative_reward = 0
...
if __name__ == "__main__":
	...
	state, done = env.reset(env_name="env",shape_keys=requested_shapes), False
	...
	for t in range(int(args.max_episode)):
		...
		if (t + 1) % args.eval_freq == 0:
			eval_ret = eval_policy(policy, args.env_name, args.seed, requested_shapes, eval_episodes=200, compare=True)
