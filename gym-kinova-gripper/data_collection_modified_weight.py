...
def getRandomJoint(obj_size, obj_pose, obj_shape):
    ...
    if obj_size == "S":
        # Two fingered side
        if obj_pose == -0.02:
            f2 = np.random.uniform(0, 0.65)
            f3 = np.random.uniform(0, 0.65)
            f1 = np.random.uniform(0, 1.15)
        # One fingered side
        if obj_pose == 0.02:
            f2 = np.random.uniform(0, 1.15)
            f3 = np.random.uniform(0, 1.15)
            f1 = np.random.uniform(0, 0.65) 
        if obj_pose == -0.03:
            f2 = np.random.uniform(0, 0.55)
            f3 = np.random.uniform(0, 0.55)
            f1 = np.random.uniform(0, 1.15)
        # One fingered side
        if obj_pose == 0.03:
            f2 = np.random.uniform(0, 1.15)
            f3 = np.random.uniform(0, 1.15)
            f1 = np.random.uniform(0, 0.55) 
        if obj_pose == -0.04:
            f2 = np.random.uniform(0, 0.35)
            f3 = np.random.uniform(0, 0.35)
            f1 = np.random.uniform(0, 1.15)
        # One fingered side
        if obj_pose == 0.04:
            f2 = np.random.uniform(0, 1.15)
            f3 = np.random.uniform(0, 1.15)
            f1 = np.random.uniform(0, 0.35)                         
        # Center
...
def DataCollection_GraspClassifier(episode_num, obj_shape, obj_size, save=False):
    ...
    for episode in range(episode_num):
        ...
        while not done:
            ...
            if (np.max(np.abs((np.array(obs[25:28]) - target_joint_config))) > 0.1) and (reach == False):
    ...
    if save:
        filename = "/home/graspinglab/NCS_data/GraspClassifier/Data" + "_{}".format(obj_shape) + "_{}".format(obj_size)
        print("Saving...")
        data = {}
        print(np.sum(graspSuccess_label))
