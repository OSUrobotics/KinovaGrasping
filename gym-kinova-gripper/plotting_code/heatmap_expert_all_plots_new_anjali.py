...
def heatmap_combined_fail(x_success,y_success,x_fail,y_fail,a,b,plot_title,img_name):
  ...
  plt.title(title)
  plt.xlabel('Initial X-coordinate of object (meters)')
  plt.ylabel('Initial Y-coordinate of object (meters)')
  plt.minorticks_off()
...
def heatmap_combined_success(x_success,y_success,x_fail,y_fail,x_total,y_total,plot_title,img_name):
  ...
  h2, x_edges, y_edges = np.histogram2d(x_total, y_total, range=[[-.09, 0.09], [0, 0.09]],bins=(x_bins,y_bins))
...
def create_plots(success_datax,success_datay,fail_datax,fail_datay,total_datax,total_datay):
    ...
    fail_plot_title = "Grasp Trial Failure Rate per Initial Pose of Object ("+shapes+") - Naive Controller"
