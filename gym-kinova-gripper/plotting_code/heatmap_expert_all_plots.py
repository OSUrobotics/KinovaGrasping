...
def heatmap_combined_fail(x_success,y_success,x_fail,y_fail,a,b,plot_title,img_name):
  ...
  plt.imshow(g3.T, cmap=plt.cm.RdBu,  origin='lower',extent=[-.09, 0.09, 0, 0.07],vmin=-1, vmax=1) #blues, failure
  ax.set_aspect('equal', adjustable='box')
...
def heatmap_combined_success(x_success,y_success,x_fail,y_fail,x_total,y_total,plot_title,img_name):
  ...
  plt.imshow(h3.T, cmap=plt.cm.RdBu,  origin='lower',extent=[-.09, 0.09, 0, 0.07],vmin=-1, vmax=1) #blues, failure
  ax.set_aspect('equal', adjustable='box')
...
def create_plots(success_datax,success_datay,fail_datax,fail_datay,total_datax,total_datay):
    ...
    fail_plot_title = "Grasp Trial Success Rate per Initial Coordinate Position of Object ("+shapes+") - Expert PID"
    ...
    success_plot_title = "Grasp Trial Success Rate per Initial Coordinate Position of Object ("+shapes+") - Expert PID"
