...
def DataCollection_GraspClassifier(episode_num, obj_shape, obj_size, save=True):
	...
	for episode in range(episode_num):
		...
		while not done:
			...
			# Second part : close fingers
				...
				if step >= 50 and step < 100: # wait for one second
					episode_obs_label.append(obs) # collect observation data after reach random joint config
		...
		# If object is lifted, 	
		for i in range(len(episode_obs_label)):
			if reward:
				graspSuccess_label.append(1)
			else:
				graspSuccess_label.append(0)
			obs_label.append(episode_obs_label[i])
